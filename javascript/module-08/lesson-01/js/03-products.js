...
productsEl.innerHTML = renderProducts(products);

productsEl.addEventListener("click", handleProductClick);

function renderProducts(products = []) {
  return products
    .map(product => {
      return `
      <li class="products-item" id="${product.id}">
        <img
...
function handleProductClick(event) {
  const cardEl = event.target.closest(".products-item");
  if (!cardEl) return;
  ...
  const modalOptions = { closable: false };

  const instance = basicLightbox.create(
    `<div class="modal">
      <img src="${searchedProduct.thumbnail}" alt="${searchedProduct.title}" />
      <h2>Title: ${searchedProduct.title}</h2>
      <h3>Description: ${searchedProduct.description}</h3>
      <p>Rating: ${searchedProduct.rating}</p>
    </div>
  `,
    modalOptions
  );
